...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 8), num_res_blocks, attn_resolutions, dropout=0.0,
                 resamp_with_conv=True, in_channels, resolution, z_channels, double_z=True, use_linear_attn=False,
                 attn_type="vanilla", **ignore_kwargs):
        """

        :param ch: (batch,image_channel(3),H,W) -> (batch,ch,H,W)
        :param out_ch: final output
        :param ch_mult: each resblock out = ch*ch_mult
        :param num_res_blocks:
        :param attn_resolutions: if resolution matched this, create attention layer, otherwise, no attention layer.
        :param dropout: dropout of each Resnetblock
        :param resamp_with_conv: whether add conv layer to downsample block
        :param in_channels:
        :param resolution:
        :param z_channels: final conv layer
        :param double_z: if true, z_channels*2
        :param use_linear_attn: True, attn_type=linear
        :param attn_type: ["vanilla", "linear", "none"]
        :param ignore_kwargs:
        """
        super().__init__()
        if use_linear_attn:
