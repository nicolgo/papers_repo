...
from ldm.modules.diffusionmodules.util import (linear, conv_nd, checkpoint, normalization, avg_pool_nd, zero_module,
                                               timestep_embedding, )
...
class QKVAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        scale = 1 / math.sqrt(math.sqrt(ch))
        weight = th.einsum("bct,bcs->bts", (q * scale).view(bs * self.n_heads, ch, length),
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        return checkpoint(self._forward, (x,), self.parameters(),
                          True)  # TODO: check checkpoint usage, is True # TODO: fix the .half call!!!  # return pt_checkpoint(self._forward, x)  # pytorch
...
class UNetModel(nn.Module):
    ...
    def __init__(self, image_size, in_channels, model_channels, out_channels, num_res_blocks, attention_resolutions,
        ...
        if use_spatial_transformer:
            assert context_dim is not None, 'not include the dimension of your cross-attention conditioning...'

        if context_dim is not None:
            assert use_spatial_transformer, 'not use the spatial transformer for your cross-attention conditioning...'
        ...
        self.time_embed = nn.Sequential(linear(model_channels, time_embed_dim), nn.SiLU(),
            ...
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(AttentionBlock(ch, use_checkpoint=use_checkpoint, num_heads=num_heads,
                                                 ...
                                                 use_new_attention_order=use_new_attention_order, ) if not use_spatial_transformer else SpatialTransformer(
            ...
            for i in range(num_res_blocks + 1):
                ...
                if level and i == num_res_blocks:
                    ...
                    layers.append(ResBlock(ch, time_embed_dim, dropout, out_channels=out_ch, dims=dims,
                                           use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                                           up=True, ) if resblock_updown else Upsample(ch, conv_resample, dims=dims,
                                                                                       out_channels=out_ch))
                    ds //= 2
