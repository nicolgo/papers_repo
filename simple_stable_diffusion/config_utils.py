...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, train=None, validation=None, test=None, predict=None, wrap=False, num_workers=None,
        ...
        # set num_workers to zero on windows. Nicolas G.
    ...
    def _train_dataloader(self):
        ...
        if is_iterable_dataset or self.use_worker_init_fn:
            ...
            init_fn = None
        # disable shuffle to avoid IndexError: list index out of range. Nicolas G.
...
def configure_logger(logdir, model, lightning_config, ckptdir, nowname, trainer_opt, opt, config, cfgdir, now):
    # default logger configs
    default_logger_cfgs = {"wandb": {"target": "pytorch_lightning.loggers.WandbLogger",
                                     "params": {"name": nowname, "save_dir": logdir, "offline": opt.debug,
                                                "id": nowname, }},
                           "testtube": {"target": "pytorch_lightning.loggers.TestTubeLogger",
    ...
    default_modelckpt_cfg = {"target": "pytorch_lightning.callbacks.ModelCheckpoint",
                             "params": {"dirpath": ckptdir, "filename": "{epoch:06}", "verbose": True,
                                        "save_last": True, }}
    ...
    default_callbacks_cfg = {"setup_callback": {"target": "config_utils.SetupCallback",
                                                "params": {"resume": opt.resume, "now": now, "logdir": logdir,
                                                           "ckptdir": ckptdir, "cfgdir": cfgdir, "config": config,
                                                           "lightning_config": lightning_config, }},
                             "image_logger": {"target": "config_utils.ImageLogger",
                                              "params": {"batch_frequency": 750, "max_images": 4, "clamp": True}},
                             "learning_rate_logger": {"target": "config_utils.LearningRateMonitor",
                                                      "params": {"logging_interval": "step",  # "log_momentum": True
                                                                 }},
                             "cuda_callback": {"target": "config_utils.CUDACallback"}, }
    if version.parse(pl.__version__) >= version.parse('1.4.0'):
        ...
        default_metrics_over_trainsteps_ckpt_dict = {
            'metrics_over_trainsteps_checkpoint': {"target": 'pytorch_lightning.callbacks.ModelCheckpoint',
                                                   'params': {
                                                       ...
                                                       "filename": "{epoch:06}-{step:09}", "verbose": True,
                                                       'save_top_k': -1, 'every_n_train_steps': 10000,
...
def configure_learning_rate(config, model, lightning_config, opt, cpu):
