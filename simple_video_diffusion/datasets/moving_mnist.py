...
def download(root, filename):
    ...
    urlretrieve(filename, os.path.join(root, os.path.basename(filename)))
...
def video_tensor_to_gif(tensor, path, duration = 120, loop = 0, optimize = True):
    images = map(torchvision.transforms.ToPILImage(), tensor.unbind(dim = 1))
    first_img, *rest_imgs = images
    first_img.save(path, save_all = True, append_images = rest_imgs, duration = duration, loop = loop, optimize = optimize)
    return images

if __name__=="__main__":
    dl = mmnist_data_loader(n_frames=20,n_workers=0,num_digits=2)
    batch = next(iter(dl))
    videos = batch[0]
    videos = videos.permute(1,0,2,3)
    video_tensor_to_gif(tensor=videos,path="sample.gif")