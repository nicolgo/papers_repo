...
# dataset has 55 groups, each group contains [train/test/val] datasets
class ShapeNetCore(Dataset):
    ...
    def __init__(self, path, cates, split, scale_mode, transform=None):
        ...
        assert isinstance(cates, list)
        assert split in self.groups
    ...
    def load(self):
        def _enumerate_pointclouds(f):
            for syssetid in self.cate_synsetids:
                cate_name = synsetid_to_cate[syssetid]
                for j, pc in enumerate(f[syssetid][self.split]):
