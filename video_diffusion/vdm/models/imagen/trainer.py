...
class ImagenTrainer(nn.Module):
    ...
    def validate_and_set_unet_being_trained(self, unet_number=None):
        ...
        assert not exists(
            self.only_train_unet_number) or self.only_train_unet_number == unet_number, 'you cannot only train on one unet at a time. you will need to save the trainer into a checkpoint, and resume training on a new unet'
